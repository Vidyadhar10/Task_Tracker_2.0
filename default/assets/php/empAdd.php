...
if (mysqli_query($con, $sql)) {
	...
	$Activity_Text = mysqli_real_escape_string($con, "A new employee <span class='text-success'>$EmpNm</span> has been added to the system by <span class='text-success'>$createrNames</span>");
	...
	$getEmpIDforLeave = "SELECT * FROM `employeedata` WHERE `MobileNo`= '$EmpMob'";
	$executeForLeave = $con->query($getEmpIDforLeave);
	while ($getInfoForLeave = $executeForLeave->fetch_assoc()) {
		$EmpIDforLeave = $getInfoForLeave['ID'];
	}

	$query = "INSERT INTO `empleave` (`id`,$columns) VALUES ($EmpIDforLeave,$values)";
	$con->query($query);
